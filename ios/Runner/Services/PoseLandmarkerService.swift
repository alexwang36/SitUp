...
class PoseLandmarkerService: NSObject {
  ...
  private func createPoseLandmarker() {
    print("[PoseLandmarkerService] Creating PoseLandmarker with model: \(modelPath)")
    print("[PoseLandmarkerService] Running mode: \(runningMode)")
    print("[PoseLandmarkerService] Delegate: \(delegate)")
    ...
    do {
      ...
      print("[PoseLandmarkerService] Successfully created PoseLandmarker")
    }
    catch {
      print("[PoseLandmarkerService] ERROR creating PoseLandmarker: \(error)")
      print("[PoseLandmarkerService] Error details: \(error.localizedDescription)")
    }
  }
  ...
  func detect(image: UIImage) -> ResultBundle? {
    print("[PoseLandmarkerService] Attempting to convert UIImage to MPImage...")
    print("[PoseLandmarkerService] UIImage size: \(image.size)")
    print("[PoseLandmarkerService] UIImage scale: \(image.scale)")
    
    guard let mpImage = try? MPImage(uiImage: image) else {
      print("[PoseLandmarkerService] ERROR: Failed to convert UIImage to MPImage")
      return nil
    }
    
    print("[PoseLandmarkerService] Successfully converted to MPImage")
    print("[PoseLandmarkerService] poseLandmarker exists: \(poseLandmarker != nil)")
    
    guard let poseLandmarker = self.poseLandmarker else {
      print("[PoseLandmarkerService] ERROR: poseLandmarker is nil")
      return nil
    }
    
    do {
      print("[PoseLandmarkerService] Starting detection...")
      let startDate = Date()
      let result = try poseLandmarker.detect(image: mpImage)
      let inferenceTime = Date().timeIntervalSince(startDate) * 1000
      print("[PoseLandmarkerService] Detection completed in \(inferenceTime)ms")
      print("[PoseLandmarkerService] Result count: \(result.landmarks.count)")
      return ResultBundle(inferenceTime: inferenceTime, poseLandmarkerResults: [result])
    } catch {
        print("[PoseLandmarkerService] ERROR during detection: \(error)")
        print("[PoseLandmarkerService] Error details: \(error.localizedDescription)")
        if let nsError = error as NSError? {
          print("[PoseLandmarkerService] Error domain: \(nsError.domain)")
          print("[PoseLandmarkerService] Error code: \(nsError.code)")
          print("[PoseLandmarkerService] Error userInfo: \(nsError.userInfo)")
        }
        return nil
    }
  }
